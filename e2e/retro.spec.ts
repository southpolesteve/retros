...
test.describe('Retro Creation and Joining', () => {
  test('creator becomes facilitator', async ({ page }) => {
    ...
    await expect(page.locator('.participant')).toContainText('Alice (Facilitator)');
  ...
  test('second user joins as participant, not facilitator', async ({ browser }) => {
    ...
    await expect(page1.locator('.participant')).toContainText('Facilitator (Facilitator)');
    ...
    await expect(page2.locator('#joinModalTitle')).toHaveText('Multi User Test');
    ...
    await expect(page2.locator('.participant.facilitator')).toContainText('Facilitator (Facilitator)');
    await expect(page2.locator('.participant:not(.facilitator)')).toContainText('Participant');
  ...
  test('page refresh reconnects with same identity', async ({ page }) => {
    ...
    await page.click('#joinBtn');
    await expect(page.locator('#mainContent')).toBeVisible();
    await expect(page.locator('.participant')).toContainText('RefreshUser (Facilitator)');
